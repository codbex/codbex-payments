angular.module('page', ['blimpKit', 'platformView', 'platformLocale', 'EntityService'])
	...
	.controller('PageController', ($scope, $http, EntityService, Extensions, LocaleService, ButtonStates) => {
		...
		let translated = {
			yes: 'Yes',
			no: 'No',
			deleteConfirm: 'Are you sure you want to delete EmployeePayment? This action cannot be undone.',
			deleteTitle: 'Delete EmployeePayment?'
		};
		...
		LocaleService.onInit(() => {
			...
			translated.deleteTitle = LocaleService.t('codbex-payments:codbex-payments-model.defaults.deleteTitle', { name: '$t(codbex-payments:codbex-payments-model.t.EMPLOYEEPAYMENT)' });
			translated.deleteConfirm = LocaleService.t('codbex-payments:codbex-payments-model.messages.deleteConfirm', { name: '$t(codbex-payments:codbex-payments-model.t.EMPLOYEEPAYMENT)' });
		});
		$scope.dataPage = 1;
		$scope.dataCount = 0;
		$scope.dataOffset = 0;
		...
		Dialogs.addMessageListener({ topic: 'codbex-payments.EmployeePayment.EmployeePayment.entitySearch', handler: (data) => {
			resetPagination();
			...
			EntityService.count(filter).then((resp) => {
				...
				EntityService.search(filter).then((response) => {
					...
					Dialogs.showAlert({
						...
						message: LocaleService.t('codbex-payments:codbex-payments-model.messages.error.unableToLF', { name: '$t(codbex-payments:codbex-payments-model.t.EMPLOYEEPAYMENT)', message: message }),
				...
				Dialogs.showAlert({
					title: LocaleService.t('codbex-payments:codbex-payments-model.t.EMPLOYEEPAYMENT'),
					message: LocaleService.t('codbex-payments:codbex-payments-model.messages.error.unableToCount', { name: '$t(codbex-payments:codbex-payments-model.t.EMPLOYEEPAYMENT)', message: message }),
					type: AlertTypes.Error
				});
				console.error('EntityService:', error);
			});
			...
			Dialogs.showDialog({
				...
				if (buttonId === 'delete-btn-yes') {
					EntityService.delete(id).then(() => {
						...
						Dialogs.showAlert({
							title: LocaleService.t('codbex-payments:codbex-payments-model.t.EMPLOYEEPAYMENT'),
							message: LocaleService.t('codbex-payments:codbex-payments-model.messages.error.unableToDelete', { name: '$t(codbex-payments:codbex-payments-model.t.EMPLOYEEPAYMENT)', message: message }),
							type: AlertTypes.Error
						});
						console.error('EntityService:', error);
					});
