...
const dao = require("codbex-payments/gen/dao/PaymentsSent/PurchaseInvoiceLink");
const http = require("codbex-payments/gen/api/utils/http");
...
rs.service()
	...
	.resource("count/{Payment}")
		.get(function(ctx, request) {
			let Payment = parseInt(ctx.pathParameters.Payment);
			Payment = isNaN(Payment) ? ctx.pathParameters.Payment : Payment;
			http.sendResponseOk("" + dao.count(Payment));
		})
		.catch(function(ctx, error) {
            if (error.name === "ForbiddenError") {
                http.sendForbiddenRequest(error.message);
	...
	.resource("{id}")
		.get(function(ctx) {
			...
			if (entity) {
				...
				http.sendResponseNotFound("PurchaseInvoiceLink not found");
			}
		})
		.produces(["application/json"])
		.catch(function(ctx, error) {
	...
	.resource("")
		.post(function(ctx, request, response) {
			...
			response.setHeader("Content-Location", "/services/js/codbex-payments/gen/api/PurchaseInvoiceLink.js/" + entity.Id);
